...
def train(args):
    ...
    model, optimizer = ms.build(sch, target="deepspeed", config=ds_config_dict, loss_fn=nn.CrossEntropyLoss())
    model.half()
    model.cuda()
    report_memory(rank)
    ...
    inputs = (bert_input_dict["input_ids"], bert_input_dict["attention_mask"], bert_input_dict["token_type_ids"])
    loader = RepeatingLoader([inputs, bert_input_dict["labels"]])
