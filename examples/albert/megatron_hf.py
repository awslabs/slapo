...
from megatron.model.bert_model import post_language_model_processing
...
def get_model(
    ...
    if "slapo" in impl:
        ...
        from slapo.model_schedule.albert import schedule_model
        ...
        model = AlbertModel(config=config, add_pooling_layer=add_pooling_layer)
        if fp16:
            model.half()
        model.cuda()
...
def model_provider(pre_process=True, post_process=True):
    ...
    # TODO: Fix AlbertHead = MLM + SOP
    class AlbertLMHead(MegatronModule):
        """Masked LM head for Bert
        # https://github.com/huggingface/transformers/blob/v4.25.1/src/transformers/models/albert/modeling_albert.py#L873
        ...
        def __init__(
            ...
            super(AlbertLMHead, self).__init__()
            ...
            self.dense = get_linear_layer(hidden_size, embedding_size, init_method)
    ...
    class AlbertWithLMHead(torch.nn.Module):
        def __init__(self, add_pooling_layer):
            ...
            self.lm_head = AlbertLMHead(
                self.albert.embeddings.word_embeddings.weight.size(0),  # vocab_size
                self.albert.embeddings.word_embeddings.weight.size(1),  # embedding_size
                args.hidden_size,
                init_method,
        ...
        def forward(
            ...
            output_tensor = post_language_model_processing(
                ...
                self.binary_head,
                labels,
                self.albert.embeddings.word_embeddings.weight,
                args.fp16_lm_cross_entropy,
            )
            return output_tensor

    model = AlbertWithLMHead(add_pooling_layer=args.bert_binary_head)
    return model
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    print_rank_0("> building train, validation, and test datasets " "for ALBERT ...")
