...
def get_model(
    ...
    if "slapo" in impl:
        ...
        (model, _), sch = slapo.build(sch, init_weights=model._init_weights)
        report_memory()
        ...
        model = RobertaModel(config=config, add_pooling_layer=add_pooling_layer)
        if fp16:
            model.half()
        model.cuda()
...
def model_provider(pre_process=True, post_process=True):
    ...
    class RoBERTaWithLMHead(torch.nn.Module):
        def __init__(self, add_pooling_layer):
            ...
            self.lm_head = BertLMHead(
                self.backbone.embeddings.word_embeddings.weight.size(0),
                args.hidden_size,
                init_method,
        ...
        def forward(
            ...
            output_tensor = post_language_model_processing(
                ...
                self.binary_head,
                labels,
                self.backbone.embeddings.word_embeddings.weight,
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    print_rank_0("> building train, validation, and test datasets " "for RoBERTa ...")
    ...
    print_rank_0("> finished creating RoBERTa datasets ...")
